<div>
    ...
    <div class="flex flex-row justify-between ">
        ...
        <div class="flex">
            <button type="button" class="text-white bg-gray-800 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800"><a href="/bandeira/cadastrar">cadastrar Bandeira </a></button>
        </div>
    </div>
                ...
                <tbody>
                    @foreach($data as $item)
                        <tr class="odd:bg-white odd:dark:bg-gray-600 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700 border-gray-200" style="background-color: #515151;">
                            ...
                            <td class="px-6 py-4">{{$item->grupoEconomico->nome}}</td>
                            <td class="px-6 py-4">{{$item->created_at}}</td>
                            <td class="px-6 py-4">{{$item->updated_at}}</td>
                            <td class="px-6 py-4"><a href="{{'/bandeira/'.$item->id.'/editar'}}">Editar</a></td>
